...
//
// Google Translate (Expiremental)
//
var GoogleTranslateCustomField = function () {
	var self = this;
	self.Label = "Google Translate";
	self.ReferenceName = "GoogleTranslate";
	self.Render = function (options) {
		/// <summary>Function called whenever the form container this Custom Field Type is rendered or refreshed.</summary>
		/// <param name="options" type="Object">
		///     <field name="$elem" type="jQueryElem">The .field-row jQuery Dom Element.</field>
		...
		if ($pnl.size() == 0) {
			...
			var url = 'http://localhost:3000/admin/google-translate'; //for testing locally
			//var url = 'https://next-blog-translate-ai-content.vercel.app/admin/google-translate'; //uses a hosted, multi-tenanted endpoint for any customer, replace with your own deployed URL if you have your own version
			var iframe = document.createElement('iframe');
			iframe.className = "gt-field";
			iframe.width = '100%';
			iframe.height = '50px';
			iframe.src = url;
			iframe.onload = function () {
				console.log('Google Translate *CMS* => Iframe Loaded')
			}
			options.$elem.html(iframe);

			var item = ContentManager.ViewModels.Navigation.TopStackItem().item()

			//DON'T NEED THIS
			// item.ContentItem().ContentItemID.subscribe(function (updatedItem) {
			// 	//listen to updates when the item is updated (after save, for example)
			// 	if (! iframe || !iframe.contentWindow) {
			// 		return;
			// 	}
			// 	var newItem = ko.mapping.toJS(item.ContentItem)
			// 	iframe.contentWindow.postMessage({
			// 		message: {
			// 			contentItem: newItem
			// 		},
			...
			var contentItem = ko.mapping.toJS(item.ContentItem)
			var contentDefinitionFields = ko.mapping.toJS(item.ContentDefinitionFields)

			var referenceName = item.ContentView().ReferenceName();
			...
			window.addEventListener("message", function (e) {
				...
				switch (messageType) {
					case 'fieldIsReady':
						...
						ContentManager.DataAccess.GetLanguages(config.WebsiteName,
							function (langs) {
								const msg = {
									message: {
										auth: {
											...
											securityKey: config.SecurityKey,
											languageCode: ContentManager.ViewModels.Navigation.currentLanguageCode(),
											languageObj: ContentManager.ViewModels.Navigation.currentLanguageObj(),
											location: 'USA', //or CANADA
										},
										fieldValue: ko.unwrap(options.fieldBinding),
										custom: {
								...
								console.log('Google Translate *CMS* => Sending Auth and fieldValue message', msg);
...
ContentManager.Global.CustomInputFormFields.push(new GoogleTranslateCustomField());